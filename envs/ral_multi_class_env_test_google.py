...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_google_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = pd.read_csv(os.path.join("data","google_review_ratings.csv"))
        test_num = 1000
        all_num = len(df)
        train_num = 10
        df = df[df.columns[df.columns!='User']]
        df = df[df.columns[df.columns!='Unnamed: 25']]
        df['Category 24'].fillna(df['Category 24'].median(), inplace=True)
        df['Category 12'].fillna(df['Category 12'].median(), inplace=True)
        df.loc[df['Category 11'] == '2\t2.', 'Category 11'] = 2.0
