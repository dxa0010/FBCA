...
class LalEnv(gym.Env):
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = pd.DataFrame(df)
        df['Y'] = np.load("data/y_auto_two.npy")
    ...
    def render(self, mode='human', close=False): # 環境を可視化する
        outfile = sys.stdout
        outfile.write("\rtrain_size:{0}, mse_test:{1}, r2_test:{2}\n".format(len(self.df_train), self.mse_test, self.r2_test))
        return outfile
