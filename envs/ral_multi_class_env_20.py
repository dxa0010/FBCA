...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def __init__(self):
        super().__init__()
        print("20________________________________________________")
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = np.load(os.path.join("data","X_20_class_15_feature_10000_n_sample.npy"))
        df = pd.DataFrame(df)
        test_num = 1000
        all_num = len(df)
        train_num = np.random.randint(10, 1000)
        df['Y'] = np.load(os.path.join("data","y_20_class_15_feature_10000_n_sample.npy"))
        df['Y']  = df['Y'].map(lambda x: 1 if x>9 else 0)  #2019 12/29 edit
