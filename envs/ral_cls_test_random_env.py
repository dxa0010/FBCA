...
class LalClEnv(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result", "AL_test_random_log_Cl_" + str(self.epsode_num) + ".csv")
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        test_num = 1000
        train_num = 100
        df = np.load("data/X_auto_two.npy")
        df = pd.DataFrame(df)
        df['Y'] = np.load("data/y_auto_two_C.npy")
    ...
    def get_observe(self, action):
        ...
        in_p_y, self.y_pool = self.y_pool[0], self.y_pool[1:]
