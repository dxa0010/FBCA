...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_white_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = pd.read_csv(os.path.join("data","winequality-white.csv"), sep=';')
        test_num = 1000
        all_num = len(df)
        train_num = 200
