...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","RAL_log_multi_random_" + str(self.epsode_num) + ".csv")
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        test_num = 1000
        train_num = 100
        df = np.load(os.path.join("data","X_20_class_50_feature.npy"))
        df = pd.DataFrame(df)
        df['Y'] = np.load(os.path.join("data","y_20_class_50_feature.npy"))
        ...
        # df_name = os.path.join("data","df_mnulti_random_" + str(self.epsode_num)+ ".csv")
        # self.df.to_csv(df_name, index=False)

        self.prob_pool, self.variance_pool, self.score, self.feature = self.make_model()
    ...
    def get_observe(self, action):
        ...
        in_p, self.X_pool = self.X_pool[1], self.X_pool[2:]
        in_p_y, self.y_pool = self.y_pool[1], self.y_pool[2:]
    ...
    def make_model(self):
        ...
        return prob_pool, variance_pool, score, obs
