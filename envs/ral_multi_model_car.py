...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","{}_car_episode_{ep}_weight_{wg}.csv").format(self.name, ep=self.epsode_num, wg = self.seed_df)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df_name =  "_ral_multi_class_{ds}_{ep}.csv".format(ds = "car", ep = self.seed_df)
