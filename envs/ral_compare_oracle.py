...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_compare_oracle_trip_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
            np.savetxt(result_log_name, self.mse_test_list, fmt='%s', delimiter=',')
            result_index_name =  os.path.join("result","ral_compare_oracle_index_trip_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
            np.savetxt(result_index_name, np.array(self.ral_index), delimiter=',')

        return observation, reward, done, {}

    def reset(self): # 状態を初期化し、初期の観測値を返す
        self.epsode_num = self.epsode_num + 1
        df = pd.read_csv(os.path.join("data","tripadvisor_review.csv"))
        df = df[df.columns[df.columns!='User ID']]
        df.loc[:,'Category 1'] = df.loc[:,'Category 1'].round()
        test_num = 100
        all_num = len(df)
        train_num = 10

        self.df = df.sample(frac=1).reset_index(drop=True)
        df_test = self.df[:test_num]
        self.X_test = df_test.iloc[:, :-1].values
        self.y_test = df_test.loc[:, 'Category 1'].values
        df_train = self.df[test_num:(test_num + train_num)]
        self.X_train= df_train.iloc[:, :-1].values
        self.y_train= df_train.loc[:, 'Category 1'].values
        df_pool = self.df[(test_num + train_num):(test_num + train_num+25)]
        self.X_pool = df_pool.iloc[:, :-1].values
        self.y_pool = df_pool.loc[:, 'Category 1'].values
        self.get_num = 0
        
        self.mse_test_list = np.array([])
        df_name = "_ral_compare_oracle_trip_{}.csv".format(self.epsode_num)
