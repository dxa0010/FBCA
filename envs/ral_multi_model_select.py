...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.aq_num = 20
        self.class_num = 10
        self.name = "ral_multi_model_log_big"
        self.label = "14"
        self.ds = "adalt"
        self.reset()
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","prefix{}_{}_episode_{ep}_weight_{wg}.csv").format(self.name, self.ds, ep=self.epsode_num, wg = self.seed_df)
            # np.savetxt(result_log_name.replace("prefix", ""), self.mse_test_list, fmt='%s', delimiter=',')
            # np.savetxt(result_log_name.replace("prefix", "obs_"), np.array(self.observation_list), fmt='%s', delimiter=',')
            # np.savetxt(result_log_name.replace("prefix", "act_"), np.array(self.action_list), fmt='%s', delimiter=',')
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df_name =  "_ral_multi_class_{ds}_{ep}.csv".format(ds = self.ds, ep = self.seed_df)
        ...
        self.action_list = []
        # df_name = "_ral_multi_class_trip_{}.csv".format(self.epsode_num)
