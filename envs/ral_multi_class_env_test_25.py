...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_test_4_episode_{ep}_weight_{wh}.csv").format(ep=self.epsode_num, wh=self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = np.load(os.path.join("data","X_25_class_17_feature_10000_n_sample.npy"))
        df = pd.DataFrame(df)
        test_num = 1000
        all_num = len(df)
        train_num = 500
        df['Y'] = np.load(os.path.join("data","y_25_class_17_feature_10000_n_sample.npy"))
        ...
        df_name = os.path.join("data","df_ral_multi_class_test_25_" + str(self.epsode_num)+ ".csv")
