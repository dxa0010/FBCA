...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_adalt_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = pd.read_csv(os.path.join("data","adult.csv"),header=None)
        adalt_name_attr = [1,3,5,6,7,8,9,13,14]
        for i in adalt_name_attr:
            le = LabelEncoder()
            le = le.fit(df.loc[:, i])
            df.loc[:, i] = le.transform(df.loc[:,i])
        test_num = 1000
        all_num = len(df)
        train_num = np.random.randint(5, 100)
        self.df = df.sample(frac=1).reset_index(drop=True)
        df_test = self.df[:test_num]
        self.X_test = df_test.iloc[:, :-1].values
        self.y_test = df_test.loc[:, 14].values
        df_train = self.df[test_num:(test_num + train_num)]
        self.X_train= df_train.iloc[:, :-1].values
        self.y_train= df_train.loc[:, 14].values
        df_pool = self.df[(test_num + train_num):]
        self.X_pool = df_pool.iloc[:, :-1].values
        self.y_pool = df_pool.loc[:, 14].values
        self.get_num = 0
        
        self.mse_test_list = np.array([])

        # df_name = "_ral_multi_class_adalt_{}.csv".format(self.epsode_num)
