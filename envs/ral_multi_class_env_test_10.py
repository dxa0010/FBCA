...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_test_2_episode_{ep}_weight_{wh}.csv").format(ep=self.epsode_num, wh=self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = np.load(os.path.join("data","X_10_class_8_feature_10000_n_sample.npy"))
        df = pd.DataFrame(df)
        test_num = 1000
        all_num = len(df)
        train_num = 200
        df['Y'] = np.load(os.path.join("data","y_10_class_8_feature_10000_n_sample.npy"))
        self.df = df.sample(frac=1, random_state=self.epsode_num).reset_index(drop=True)
        ...
        df_name = os.path.join("data","df_ral_multi_class_test_10_" + str(self.epsode_num)+ ".csv")
    ...
    def make_model(self):
        ...
        rfc = RandomForestClassifier(n_estimators=500, max_depth=10, oob_score= True, random_state=0).fit(X_train, y_train) 
