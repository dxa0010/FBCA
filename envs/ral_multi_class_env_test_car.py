...
class RAL_Multi_Class_Env(gym.Env):
    ...
    def step(self, action): # actionを実行し、結果を返す
        ...
        if done:
            result_log_name =  os.path.join("result","ral_multi_class_log_car_episode_{ep}_weight_{wg}.csv").format(ep=self.epsode_num, wg = self.weight_num)
    ...
    def reset(self): # 状態を初期化し、初期の観測値を返す
        ...
        df = pd.read_csv(os.path.join("data","car.csv"),header=None)
        car_name_attr = [0,1,4,5,6]

        for i in car_name_attr:
            le = LabelEncoder()
            le = le.fit(df.loc[:, i])
            df.loc[:, i] = le.transform(df.loc[:,i])
        df.loc[:,3]=df.loc[:, 3].str.replace('more','6')
        df.loc[:,2]=df.loc[:, 2].str.replace('5more','6')
